	...
	def get_fieldsets(self, request, obj=None):
		if request.user.is_superuser:
			...
			return [(None, {'fields':(
				...
				'important_change', 'productlist', 'working_skill', 'organize_framework','working_skill_radar',
				'standbook', 'process_layout', 'dagerous_map', 'improve_proposal',
				'attainment_rate', 'pass_rate','exception','typical_question')})]
...
class ProductExecutingAdmin(admin.ModelAdmin):
	list_display = ('product', 'produce_date','excuted', 'plan')
	...
	def formfield_for_foreignkey(self, db_field, request, **kwargs):
		...
		if db_field.name == 'Area' :
			kwargs["queryset"] = Area.objects.filter(managerList__id=request.user.id)
		elif db_field.name == 'product':
			kwargs["queryset"] = Product.objects.filter(area__managerList__id=request.user.id).distinct()
		return super(ProductExecutingAdmin, self).formfield_for_foreignkey(db_field,request,**kwargs)			
