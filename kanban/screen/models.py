...
class Area(models.Model):
	"工区管理"
	# general 
	managerList = models.ManyToManyField(User, verbose_name= "管理人员", null=True,blank=True)
	...
	# page1 工区简介
	introduce = models.CharField(max_length=1000, verbose_name='介绍,不超过500个字',blank=True)
	process_discipline = models.ImageField('工艺纪律检查图片', upload_to='process_discipline',blank=True)
	# security_cross
	important_change = models.ImageField('重大更改或整改图片', upload_to='important_change',blank=True)

	#page2 工区域的产品
	productlist = models.ManyToManyField(Product, verbose_name='能生产的产品列表',blank=True)

	#page4 人员组织和技能
	working_skill = models.ImageField('班组人员职责以及技能', upload_to='working_skill',blank=True)
	organize_framework = models.ImageField('工区组织架构图', upload_to='organize_framework',blank=True)
	working_skill_radar = models.ImageField('人员技能统计图' , upload_to='working_skill_radar',blank=True)

	#page5 
	standbook = models.ImageField('工位台账信息', upload_to='standbook',blank=True)

	#page6
	process_layout = models.ImageField('工区工艺布局图', upload_to='process_layout',blank=True)

	#page7
	dagerous_map = models.ImageField('工位危险源地图', upload_to='dagerous_map',blank=True)

	#page8
	improve_proposal = models.ImageField('改善提案图片', upload_to='improve_proposal',blank=True)
	attainment_rate = models.DecimalField('标准工位达标率,单位百分比', max_digits=4, decimal_places=2,blank=True)

	#page9
	pass_rate = models.ImageField('当月一次性合格率统计', upload_to='pass_rate',blank=True)
	exception = models.ImageField('当月异常情况', upload_to='exception',blank=True)
	typical_question = models.ImageField('典型质量问题', upload_to='typical_question',blank=True)
